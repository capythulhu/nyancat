...
## Recommended IDE
I recommend [Visual Studio Code](https://code.visualstudio.com/) for you to write your quantum algorithms. I've prepared an extension that you might want to use so you can have stuff like syntax highlighting and snippets! Get it here: [nyancat VSCode Extension](https://marketplace.visualstudio.com/items?itemName=thzoid.nyancat).

# What in the world is this quantum stuff?
I'm working on some lazy and very superficial explanation on quantum computing but I'll always recommend the [Microsoft's documentation on this subject](https://docs.microsoft.com/en-us/quantum/overview/understanding-quantum-computing). [IBM's documentation for quantum computing](https://quantum-computing.ibm.com/docs/guide/) is also a masterpiece when it comes to explaining what is this crazy stuff called qubit. I want to mention that almost every effort I've put on this work was based on these two documentations and their SDKs. However, I felt that there was very few material about low-level quantum programming. That's why I started to develop this humble library. Anyways, let's get this going!
...
The string `mov` is what addresses what task you're calling. Each task has different parameters count and types &mdash; this one recieves a classical pointer and a value &mdash; and the parameters are separated by a whitespace character. **There can only be one task call per line.**
...
| div    | Divides (floor) the value of the classical register associated with the provided pointer to the provided value.                   | classical_pointer value              | No            | No
...
Let's start by analyzing the quantum code. All we do in that file is perform a Hadamard gate on the qubit (`0?`) &mdash; a.k.a. put it in superposition &mdash; and then measure it. This will force the qubit to randomly be set to `0` or `1`. Then, we end the algorithm outputting the value to the TRR (`0%`), which is the result of the measurement.

On the C code, we allocate a driver that has 1 quantum register (we only need that one qubit) and 0 classical register (we're not doing any classical operation with registers on our `.nya` code). Then, we load the script into a `qscript` structure, which cointains the parsed script itself and the minimum count of classical and quantum registers that must be allocated in order to that algorithm work (yes, we can input these on the driver's "constructor", and you should do this if your purpose is to run one algorithm only).
